...
def create_user_defined_chart(n_index, def_x_idx, def_y_idx, def_color_idx):
...
with st.sidebar.expander('Default variables for charts'): 
    ...
    default_Y = st.selectbox('Default Y/Target variable:', df.columns)
    color_options = ['None']
    color_options.extend(df.columns.tolist())
...
for i in range(int(chart_num)):
    with st.sidebar.expander(f'Chart {i+1}'):
        fig = create_user_defined_chart(i, def_x_idx, def_y_idx, def_color_idx)
